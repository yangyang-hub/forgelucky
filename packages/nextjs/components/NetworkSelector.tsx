...
export const NetworkSelector = () => {
  ...
  const handleNetworkSwitch = (chainId: number) => {
    if (switchChain && chainId !== currentChain?.id) {
      switchChain({ chainId });
    }
    setIsOpen(false);
  };
  ...
  return (
    <div className="dropdown dropdown-bottom dropdown-end">
      <div tabIndex={0} className="btn btn-sm btn-outline gap-2 normal-case" onClick={() => setIsOpen(!isOpen)}>
        {currentNetworkInfo && (
          <>
      ...
      {isOpen && (
        ...
        >
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <div className={`font-medium ${getStatusColor(network.id)}`}>{networkInfo.label}</div>
                      <div className="text-sm opacity-60">{networkInfo.description}</div>
                      <div className="text-xs opacity-40">Chain ID: {network.id}</div>
                    </div>
                  </div>
          ...
          <li className="menu-title mt-2">
            <span className="text-xs opacity-60">💡 Switch networks to deploy contracts on different chains</span>
          </li>
        </ul>
      )}
    </div>
