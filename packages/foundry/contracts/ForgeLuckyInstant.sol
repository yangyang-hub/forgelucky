    ...
    ReentrancyGuard
{
    ...
    /// @notice 超级大奖池占比：30%
    uint256 public constant SUPER_GRAND_POOL_RATIO = 3000;
    ...
    // 超级大奖单次中奖抽取奖金池资金比例 50%
    uint256 public constant SUPER_GRAND_FUND_RATIO = 5000;

    // 大奖单次中奖抽取奖金池资金比例 12.5%
    uint256 public constant GRAND_PRIZE_FUND_RATIO = 1250;

    // 中奖单次中奖抽取奖金池资金比例4%
    uint256 public constant MEDIUM_PRIZE_FUND_RATIO = 400;

    // 小奖单次中奖抽取奖金池资金比例1.5%
    uint256 public constant SMALL_PRIZE_FUND_RATIO = 150;
    ...
    struct PrizePool {
        ...
        uint256 currentFunds; // 奖池当前资金
        uint256 totalTickets; // 总彩票数
        uint256 totalUnclaimedTickets; // 未开奖彩票数
    ...
    struct SystemStats {
        ...
        uint256 totalGrowthFund; // 总增长基金
        uint256 currentGrowthFund; // 当前增长基金
        uint256 totalTickets; // 总彩票数
    }
    ...
    constructor(
    ...
    ) ERC721("ForgeLucky Instant Ticket", "FLIT") Ownable(msg.sender) {
        ...
        require(
            SUPER_GRAND_POOL_RATIO +
                GRAND_PRIZE_POOL_RATIO +
                MEDIUM_PRIZE_POOL_RATIO +
                SMALL_PRIZE_POOL_RATIO +
                GROWTH_FUND_RATIO +
                PLATFORM_FEE_RATE ==
                FEE_BASE,
            "Invalid allocation ratios"
        );

        // 验证中奖率为25% (2500/10000)
        uint256 totalWinRate = SUPER_GRAND_RATE +
            GRAND_PRIZE_RATE +
            MEDIUM_PRIZE_RATE +
            SMALL_PRIZE_RATE;
    ...
    function drawTicket(
        uint256 tokenId
    ) external onlyValidTicket(tokenId) nonReentrant {
     ...
     * @notice 批量开奖指定的彩票
     */
    function batchDrawSpecificTickets(
     ...
     * @notice 内部开奖逻辑
     */
    function _drawTicket(
        uint256 tokenId
    ) internal returns (PrizeLevel prizeLevel, uint256 prizeAmount) {
        // 安全检查：验证随机数生成器的状态
        require(!randomGenerator.paused(), "Random generator paused");
        require(
            randomGenerator.isAuthorized(address(this)),
            "Contract not authorized for random generation"
        );

        // 生成随机数
        uint256 randomSeed = randomGenerator.generateSecureRandom(
            tokenId,
            block.timestamp,
            block.prevrandao
        );
        ...
        if (randomValue < SMALL_PRIZE_RATE) {
            ...
            randomValue <
            SMALL_PRIZE_RATE + MEDIUM_PRIZE_RATE + GRAND_PRIZE_RATE
        ) {
            // 大奖：2.5%概率
    ...
    function _calculatePrizeAmount(
        ...
        if (prizeLevel == PrizeLevel.SUPER_GRAND) {
            // 超级大奖
            baseAmount = (availableFunds * SUPER_GRAND_FUND_RATIO) / FEE_BASE;
        } else if (prizeLevel == PrizeLevel.GRAND_PRIZE) {
            // 大奖
            baseAmount = (availableFunds * GRAND_PRIZE_FUND_RATIO) / FEE_BASE;
        } else if (prizeLevel == PrizeLevel.MEDIUM_PRIZE) {
            // 中奖
            baseAmount = (availableFunds * MEDIUM_PRIZE_FUND_RATIO) / FEE_BASE;
        } else if (prizeLevel == PrizeLevel.SMALL_PRIZE) {
            // 小奖
            baseAmount = (availableFunds * SMALL_PRIZE_FUND_RATIO) / FEE_BASE;
        }
        return baseAmount;
    }
    ...
    function _distributeFunds(uint256 amount) internal {
        ...
        // 将余数分配给超级大奖池，确保无资金丢失
        prizePools[PrizeLevel.SUPER_GRAND].totalFunds +=
            growthPerPool +
            remainder;
        ...
        )
    {
        superGrandPool =
            prizePools[PrizeLevel.SUPER_GRAND].totalFunds -
            prizePools[PrizeLevel.SUPER_GRAND].totalClaimed;
        grandPool =
            prizePools[PrizeLevel.GRAND_PRIZE].totalFunds -
            prizePools[PrizeLevel.GRAND_PRIZE].totalClaimed;
        mediumPool =
            prizePools[PrizeLevel.MEDIUM_PRIZE].totalFunds -
            prizePools[PrizeLevel.MEDIUM_PRIZE].totalClaimed;
        smallPool =
            prizePools[PrizeLevel.SMALL_PRIZE].totalFunds -
            prizePools[PrizeLevel.SMALL_PRIZE].totalClaimed;
     ...
     * @notice 获取用户未开奖的彩票
     */
    function getUserUndrawnTickets(
        address user
    ) external view returns (uint256[] memory undrawnTickets) {
     ...
     */
    function getUserUnclaimedWinningTickets(
        address user
    ) external view returns (uint256[] memory unclaimedTickets) {
        uint256[] memory userTokenIds = userTickets[user];
        uint256 count = 0;
     ...
     */
    function getUserTicketsDetails(
        address user
    )
        external
        view
